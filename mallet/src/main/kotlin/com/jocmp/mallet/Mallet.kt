...
object Mallet {
    ...
    fun flatten(
        html: String,
        baseUrl: String,
    ): Result<LinearArticle> = html.byteInputStream().use { flatten(it, baseUrl) }

    fun flatten(
        inputStream: InputStream,
        baseUrl: String,
    ): Result<LinearArticle> = Flattening().flatten(inputStream, baseUrl)
}

private class Flattening {
    ...
    fun flatten(
        ...
        val elements =
            try {
                ...
                return Result.failure(LinearizeError.ParseFailed(e))
            }

        if (!truncated) {
            return Result.success(LinearArticle(elements = elements))
        }

        return Result.success(
            LinearArticle(
                elements = elements,
                truncated = Truncation(elementCount = elementCount, charCount = charCount),
            ),
        )
    }
...
//                        }
    ...
    private fun ListBuilderScope<LinearElement>.addLimited(element: LinearElement) {
        if (truncated) return
        elementCount++
        if (elementCount > Mallet.MAX_ELEMENTS || charCount > Mallet.MAX_CHARS) {
            truncated = true
            return
        }
        add(element)
