...
//    }
                ...
                sources =
                    listOf(
                        LinearVideoSource(
                            ...
                            "https://img.youtube.com/vi/cjxnVO9RpaQ/hqdefault.jpg",
                            480,
                            360,
                            null,
                        ),
                ...
                sources =
                    listOf(
                        LinearVideoSource(
                            ...
                            "https://img.youtube.com/vi/x_m02bUxfvE/hqdefault.jpg",
                            1170,
                            658,
                            null,
                        ),
    ...
    @Test
    fun `article within limits is not truncated`() {
        val result = Mallet.flatten("<html><body><p>Hello</p></body></html>", baseUrl).getOrThrow()

        assertNull(result.truncated)
    }

    @Test
    fun `article over the element limit reports truncation`() {
        val html = "<html><body>" + "<p>x</p>".repeat(Mallet.MAX_ELEMENTS + 100) + "</body></html>"

        val result = Mallet.flatten(html, baseUrl).getOrThrow()

        val truncation = assertNotNull(result.truncated)
        assertEquals(Mallet.MAX_ELEMENTS + 1, truncation.elementCount)
        assertEquals(Mallet.MAX_ELEMENTS, result.elements.size)
    }
}
