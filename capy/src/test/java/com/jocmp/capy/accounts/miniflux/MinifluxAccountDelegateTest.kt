...
class MinifluxAccountDelegateTest {
    ...
    fun refresh_updatesEntries() = runTest {
        ...
        coEvery { miniflux.entries(starred = true) }.returns(
            Response.success(
                EntryResultSet(
                    total = 0,
                    entries = emptyList()
        ...
        coEvery { miniflux.entries(status = EntryStatus.UNREAD) }.returns(
            Response.success(
                EntryResultSet(
                    total = 1,
        ...
        assertEquals(expected = listOf(null, "Tech"), actual = taggedNames.sorted())
