...
class OfflineAssetCache(
    ...
    suspend fun download(
        ...
        return try {
            ...
            rewriteAttribute(doc, "audio", "src", articleID, articleFolder)
            ...
            embedEnclosures(doc, enclosures, articleID, articleFolder)
    ...
    private suspend fun embedEnclosures(
        doc: Document,
        ...
        val existingSources = doc.select("audio[src], video[src], source[src]")
            .mapNotNull {
                val src = it.attr("src")
                if (src.startsWith("$ASSET_HOST/offline/")) {
        ...
        for (enclosure in enclosures) {
            ...
            val hash = sha1(url)
            if (existingSources.contains(hash)) continue

            val storedName = downloadAsset(url, hash, articleFolder) ?: continue
            val localUrl = "$ASSET_HOST/offline/$articleID/$storedName"

            val tag = when {
                enclosure.type.startsWith("audio/") -> "audio"
                enclosure.type.startsWith("video/") -> "video"
                enclosure.type.startsWith("image/") -> "img"
                else -> continue
            }

            val element = doc.createElement(tag).apply {
                attr("src", localUrl)
                if (tag != "img") attr("controls", "")
            }
            doc.body().prependChild(element)
        }
    }
